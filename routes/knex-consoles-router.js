...
consolesRouter
  .route("/:id")
  .get((req, res, next) => {
    ConsolesService.getById(req.app.get("db"), req.params.id).then(
      (console) => {
  ...
  .delete(async (req, res, next) => {
    try {
      await ConsolesService.deleteGamesInConsole(
        req.app.get("db"),
        req.params.id
      );
      await ConsolesService.deleteConsole(
        req.app.get("db"),
        req.params.id
      ).then(() => {
        res.status(204).end();
  ...
  // this adds a game with a console_id
consolesRouter.route("/:id/addGame").post(jsonParser, (req, res, next) => {
  let console_id = req.params.id;
  let title = req.body.title;
  let condition = req.body.condition;
  const newGame = { console_id, title, condition };
  ConsolesService.addGameToConsole(req.app.get("db"), newGame).then((game) => {
